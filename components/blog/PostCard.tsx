...
export default function PostCard({ post }: { post: BlogPostMeta }) {
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group block bg-[#0c0c20] rounded-2xl border border-white/[0.06] hover:border-[#7C3AED]/25 hover:shadow-lg hover:shadow-[#7C3AED]/5 transition-all duration-300 overflow-hidden"
    >
      <div className="p-6">
        ...
        <h3 className="text-lg font-bold text-white mb-2 group-hover:text-[#9D6FF0] transition-colors line-clamp-2 font-[family-name:var(--font-jakarta)]">
          {post.title}
        </h3>
